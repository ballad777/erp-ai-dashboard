...
目前狀態：除一鍵 ZIP 套件外，計畫書主要功能已補齊可執行版本。前端可整批上傳多個資料集、顯示後端真實摘要、先判斷資料主題與合併策略、選擇目標欄位、選擇分析模式、手動或自動選擇模型與圖表，並產出真實 PNG 圖表、可下載程式碼檔案、Notebook、模型結果 Excel、Word 分析報告與金融指標 CSV。完整模型物件預設不保存；只有明確要求 `save_model=true` 時才會進入安全保存流程。
...
`DataUnderstandingEngine` 以目前上傳資料的欄位、型態、唯一值、缺失值、日期欄位與語意線索做通用診斷；若證據不足，會顯示「待確認資料集」，並附上 evidence、confidence 與 limitations，不會為了範例資料硬寫結論。`TargetRecommendationEngine` 會回傳 Top 5 目標欄位候選，包含任務類型、推薦原因、警告、為什麼分數沒有更高，以及可信度分數；若沒有可靠目標，會建議先做資料探索或人工確認。
...
若 API 傳入 `save_model=true`，後端會使用 `safe_save_model`，單一模型檔超過 200 MB 會立即刪除並回傳 warning。`generated_outputs/models` 會自動保留最近 5 個模型檔，並提供 storage API 檢查與清理輸出容量。
...
- 資料摘要、欄位偵測、缺失值、品質檢查與推薦目標欄位都依上傳檔案的真實資料計算，不使用靜態假資料。
- 大型資料進行模型比較時，後端會使用固定 `random_state=42` 從真實資料抽樣訓練，避免免費雲端服務因高成本模型逾時；回傳結果會顯示原始資料列數與本次模型實際使用列數。
- `customer_id`、`order_id`、`source_row_number` 等識別碼欄位，以及高基數文字欄位，會自動排除於模型訓練特徵外，避免 one-hot 矩陣爆量；原始資料、摘要與輸出檔不會被刪改。
- 自動模型推薦會依資料列數、欄位數、缺失比例與類別欄位狀態避開 SVR、SVC、KNN 等容易拖慢大型資料的模型；使用者手動選擇這類模型且資料不適合時，後端會回傳明確錯誤。
- 缺失值補值只存在於模型訓練管線內，用於讓 scikit-learn 模型可執行；原始資料摘要與下載資料仍保留真實缺失值統計。
